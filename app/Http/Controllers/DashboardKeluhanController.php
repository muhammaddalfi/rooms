...
class DashboardKeluhanController extends Controller
{
   ...
   public function home(){
        ...
        if(Auth::user()->hasRole(['sales','mitra'])){
            $tahun = date('Y');
            $bulan = date('m');
            ...
            $db_keluhan = "SELECT o.nama_olt, jk.jenis_keluhan,jk.id,
                                    ...
                                    LEFT JOIN olts o ON o.id = k.nama_olt
                                    WHERE YEAR(k.created_at) = ? AND MONTH(k.created_at) = ?
                                    AND u.id_leader = '".Auth()->user()->id."'
                                    GROUP BY o.nama_olt, jk.jenis_keluhan,jk.id
                                    ORDER BY jk.id ASC";

            $jumlah_keluhan_query = "SELECT o.nama_olt AS nama_olt,
                                        (SELECT COUNT(id) FROM keluhans WHERE YEAR(keluhans.created_at) = ? AND MONTH(keluhans.created_at) = ? 
                                        AND user_id IN (SELECT id FROM users WHERE id = o.id OR id_leader = o.id)) AS jumlah_keluhan
                                        FROM olts o
                                        ";
